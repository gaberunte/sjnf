...
```{r}
consensus = read_csv(here("data/sanger", "sjnf_all_consensus_06052023.csv")) %>% 
  clean_names() %>% 
  dplyr::select(name, sequence) %>% 
  rename(sequence_list_name = name)

contig_assign = read_csv(here("data/sanger", "sjnf_all_contigs_06052023.csv")) %>% 
  clean_names() %>% 
  rename(contig_sequence = sequence) %>% 
  left_join(consensus)

metadata = read_csv(here("data/sanger", "sjnf_all_long_06052023.csv")) %>% 
  clean_names() %>% 
