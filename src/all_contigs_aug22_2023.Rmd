...
```{r}
consensus = read_csv(here("data/sanger", "sjnf_all_consensus_aug22_2023.csv")) %>% 
  clean_names() %>% 
  dplyr::select(name, sequence) %>% 
  rename(sequence_list_name = name)

contig_assign = read_csv(here("data/sanger", "sjnf_all_contigs_aug22_2023.csv")) %>% 
  clean_names() %>% 
  rename(contig_sequence = sequence) %>% 
  left_join(consensus)

metadata = read_csv(here("data/sanger", "sjnf_all_long_aug22_2023.csv")) %>% 
  clean_names() %>% 
...
ggplot(branch_rep, aes(replicate, y = n, fill = genus))+
  geom_bar(stat="identity")+ 
  scale_fill_manual(values = colorRampPalette(brewer.pal(8, "Set2"))(19), breaks = unique(branch_rep$genus))+
  theme_minimal()+
  labs(
    x = "Replicate",
    y = "Abundance"
...
ggplot(branch_morphs, aes(as.factor(column), fill = Genus))+
  geom_bar()+ 
  scale_fill_manual(values = colorRampPalette(brewer.pal(8, "Set2"))(19), breaks = unique(branch_morphs$Genus))+
  theme_minimal()+
  labs(
    x = "Replicate",
    y = "Abundance"
...
ggplot(branch_comp %>% 
  ...
  geom_bar()+ 
  scale_fill_manual(values = colorRampPalette(brewer.pal(8, "Set2"))(19), breaks = unique(branch_comp$genus))+
  theme_minimal()+
  labs(
    x = "Tree/Direction",
    y = "Abundance"
